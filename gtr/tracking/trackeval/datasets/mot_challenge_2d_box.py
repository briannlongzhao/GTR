...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'SEQMAP_FILE': os.path.join(code_path, 'tracking/seqmaps/'),  # Directly specify seqmap file (if none use seqmap_folder/benchmark-split_to_eval)
            'SEQ_INFO': None,  # If not None, directly specify sequences to eval and their number of timesteps
            'GT_LOC_FORMAT': '{gt_folder}/{seq}/gt/gt{split_mark}.txt',  # '{gt_folder}/{seq}/gt/gt.txt'
    ...
    def __init__(self, config=None):
        ...
        split = self.config['SPLIT_TO_EVAL']
        self.split_mark = '' if 'half' not in split else '_val_half'
        if 'half' in split:
            print('GT_FOLDER', self.config['GT_FOLDER'])
            if '20' in self.config['GT_FOLDER']:
                gt_set = 'train'
            else:
                gt_set = 'trainval'
        else:
        ...
        for seq in self.seq_list:
            if not self.data_is_zipped:
                curr_file = self.config["GT_LOC_FORMAT"].format(
                    gt_folder=self.gt_fol, seq=seq, split_mark=self.split_mark)
        ...
        print('self.tracker_list', self.tracker_list)
