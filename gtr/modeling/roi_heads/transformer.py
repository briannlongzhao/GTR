...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        encoder_layer = TransformerEncoderLayer(
            ...
            dropout, activation, normalize_before, norm=norm, 
            no_ffn=no_encoder_ffn,
            no_encoder_self_att=no_encoder_self_att)
        encoder_norm = nn.LayerNorm(d_model) if (normalize_before and norm) else None
    ...
    def forward(self, src, pos_embed=None, mask=None, query_embed=None, query_inds=None):
        ...
        memory = self.encoder(
            src, src_key_padding_mask=mask, pos=pos_embed) # N x B x F
        ...
        if query_embed is None:
            ...
            tgt = torch.zeros_like(query_embed) # N x B x F

        if self.num_decoder_layers == 0:
            return tgt.unsqueeze(0).transpose(1, 2), \
                memory.permute(1, 0, 2).view(B, N, f)
        
        hs = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, query_pos=query_embed) # M x B x F
        # return hs.view(-1, B, f).permute(1, 0, 2), \
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 ...
                 no_ffn=False, no_encoder_self_att=False):
        super().__init__()
        self.no_ffn = no_ffn
