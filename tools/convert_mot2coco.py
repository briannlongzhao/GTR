...
if __name__ == '__main__':
    if sys.argv[1] == "trainval":
        ...
        print("arg should be {'trainval','test'}")
        exit()
    ...
    for split in SPLITS:
        if '20' in DATA_PATH:
            IMAGE_DIR = DATA_PATH + (split if 'half' not in split else 'train') + '/'
            data_path = DATA_PATH + (split if 'half' not in split else 'train')
            print('data_path', data_path)
        else:
            IMAGE_DIR = DATA_PATH + ('trainval/' if (HALF_VIDEO or split == 'train') else split + '/')
            data_path = DATA_PATH + (split if (not HALF_VIDEO and (split != 'train')) else 'trainval')
            print('data_path', data_path)
        out_path = OUT_PATH + '{}_conf0.json'.format(split if split != 'train' else 'fulltrain')
